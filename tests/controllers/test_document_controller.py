...
def test_create_base_document_success(document_controller):
    ...
    ), "📄 Should return a BaseDocument instance"
    assert (
        document.document_id == "CCLW.executive.9494.3809"
    ), "📄 Document ID should match"
    assert (
        document.document_name
        == "National energy -climate plan: Part A - National Plan"
    ), "📄 Document name should match"
    assert document.document_source_url == AnyHttpUrl(
        "https://app.climatepolicyradar.org/document/national-energy-climate-plan-part-a-national-plan_2e62"
