...
class LibraryManager:
    ...
    def get_metadata_for_citation(self, document_id: str, text_block_ids: list[int]) -> dict:
    ...
    def get_text_blocks_around(self, document_id: str, text_block_id: int, N: int) -> dict:
        """Get N text blocks before and after a given block for a document."""
        url = f"{self.base_url}/full_text/-/query.json"
        params = {
            "sql": f"SELECT distinct(tb.text_block_id), tb.text, tb.page_number FROM text_blocks tb WHERE tb.document_id = :document_id AND CAST(tb.text_block_id AS INTEGER) BETWEEN (:text_block_id - :N) AND (:text_block_id + :N) ORDER BY tb.text_block_id",
            "document_id": document_id,
    ...
    def get_section_containing_block(self, document_id: str, text_block_id: int) -> dict:
