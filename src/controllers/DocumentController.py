...
class DocumentController:
    ...
    def create_base_document(self, document_id: str) -> BaseDocument:
        ...
        document_row = self.metadata_df[self.metadata_df['Internal Document ID'] == document_id]
        ...
        document = BaseDocument(
            ...
            document_name=document_row['Document Title'].values[0].strip() if isinstance(document_row['Document Title'].values[0], str) else "",
            document_source_url=f"https://app.climatepolicyradar.org/document/{document_row['Document ID'].values[0].strip()}" if isinstance(document_row['Document ID'].values[0], str) else "",
            has_valid_text=True,
            document_metadata=BaseMetadata(
                    geography=document_row['Geography'].values[0].strip() if isinstance(document_row['Geography'].values[0], str) else "",
                    publication_ts=datetime.now() #TODO PROPER DATA
                ),
            translated=True if document_row['Language'].values[0] != 'English' else False
        )
