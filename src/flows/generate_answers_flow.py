...
@task
def spawn_answer_tasks(scenario: Scenario, db: Database, tag: str, query_tag: str, only_new: bool):
    ...
    if only_new:
        ...
        queries = get_unanswered_queries(db, tag, query_tag, scenario.model, scenario.prompt.prompt_template, query_prompts)
    else:
        queries = get_queries(db, query_tag)
        
    logger.info(f"💡 Generating answers for {len(queries)} queries with tag {query_tag}")
...
@task
def generate_answer_full(query: Query, scenario: Scenario, db: Database, tag: str, query_tag: str):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--only_new", type=bool, default=True, help="Only generate answers for new queries")
    parser.add_argument("--query_tag", type=str, help="Tag for selecting grouped queries to answer")
