...
def create_query_list_task(
    ...
    try:
        ...
        response = rc.generate_queries(
            document=document, scenario=scenario, seed_queries=seed_queries, tag=tag
        )

    except Exception as e:
        logger.error(f"Error generating queries for {document.document_id}: {e}")
        raise e
...
@task(
    task_run_name="generate_answers_{tag}",  # pyright: ignore
    retries=5,
    retry_delay_seconds=exponential_backoff(backoff_factor=20),
    retry_jitter_factor=1,
    timeout_seconds=120,
...
def generate_answer_task(
    query: Query, scenario: Scenario, tag: str, rc: RagController
) -> EndToEndGeneration:
    logger = get_run_logger()

    logger.info(f'💡 Generating answer for {query.uuid}: "{query.text}"')
    try:
        response = rc.run_rag_pipeline(query=query.text, scenario=scenario)
    except Exception as e:
        logger.error(
    ...
    logger.info(f"💡 Answer: {response.get_answer(False)}")
    return response


@task(
    task_run_name="generate_evals_{generation.rag_request.document_id}_{tag}",  # pyright: ignore
    retries=5,
    retry_delay_seconds=exponential_backoff(backoff_factor=20),
    retry_jitter_factor=1,
    timeout_seconds=600,
...
def run_eval_task(generation: EndToEndGeneration, eval: str, kwargs: dict) -> str:
    logger = get_run_logger()

    ec = EvaluationController()

    try:
        result = ec.evaluate(generation, eval, kwargs)
    except Exception as e:
        logger.error(
            f"🚨 Error evaluating answer for {generation.uuid} with {eval}: {e}"
        )
        raise e
