...
def plot_model_performance(df):
    model_performance = df.groupby('model')['vectara_score'].mean().sort_values(ascending=False)
...
def plot_prompt_vs_eval(df, query_prompt_column):
    df_sorted = df.groupby(query_prompt_column)['vectara_score'].median().sort_values(ascending=True).index
...
def analyze_no_answers(df, query_prompt_column):
    no_answer_condition = (df['answer'].isnull()) | (df['answer'] == '') | (df['answer'] == 'NULL')
    no_answers = df[no_answer_condition].groupby(query_prompt_column).size().sort_values(ascending=False)
...
def generate_query_word_cloud(df):
    ...
    ax.axis('off')
    ax.set_title('Word Cloud of Queries')
    plt.tight_layout()
    return fig
def main():
    ...
    if tag:
        ...
        selected_analysis = st.multiselect("Select analyses to perform:", analysis_options)
        ...
        for analysis in selected_analysis:
            if analysis == "Model Performance":
                ...
                fig, total_no_answers, percentage_no_answers = analyze_no_answers(df, 'prompt')
                st.pyplot(fig)
            ...
            elif analysis == "Word Cloud of Queries":
                st.subheader("Word Cloud of Queries")
                fig = generate_query_word_cloud(df)
                st.pyplot(fig)
